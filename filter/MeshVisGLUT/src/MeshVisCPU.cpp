...
void MeshVis::update()
{
	...
	if( m_update_time > 1000.0f / (float) m_stepsPerSecond ) // GLUT_ELAPSED_TIME is in ms!!!
	{
		...
		if( m_createParticles )
		{
			...
			//m_particles->createParticlesTest(10000);
		...
		FPS::iterations++;
		FPS::activeParticles = m_particles->getActiveParticleCount();
...
void MeshVis::motion(int x, int y)
{
	...
	else 
	{
		...
		if( mouse_buttons == 4 )
		{
			...
			{
				translate.z += (translate.z + step < -0.1) ? step : 0;
			}
		}
...
void MeshVis::reshape(int w, int h)
{
	...
	glViewport(0, 0, w, h);
    gluPerspective(45.0, (GLfloat)ratio, 0.1, 10.0);
