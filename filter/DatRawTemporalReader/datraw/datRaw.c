...
static void swapByteOrder64(void *data, unsigned long size) 
{
	unsigned long i;
	DR_ULONG v, sv;
...
static void swapByteOrder16(void *data, unsigned long size) 
{
	unsigned long i;
	DR_USHORT v, sv;
...
static void swapByteOrder(void *data, unsigned long size, int type)
{
	switch (type) {
		case DR_FORMAT_CHAR:
		...
		DatRawOptionalField **optionalFields)
{
    ...
    */
	...
	if (! (datFileText = (char*)malloc(fstatus.st_size))) {
    ...
    while (input) {
		...
		datRaw_getLine(&input, fstatus.st_size - (input - datFileText), 
        ...
        } else if (strstr(inputLine, "TIME")) {
			if (info->time != FLT_MAX) {
				datRaw_logError("DatRaw Error: Multiple times given\n");
				error = 1;
			} else if (sscanf(sep+1, "%f", &info->time) != 1) {
                error = 1;
            }
    ...
    if (info->time == FLT_MAX) {
        datRaw_logWarning("DatRaw Warning: time not given in %s\n-> assuming 0.0\n", datfile);
        info->time = 0.0;
    }	
	...
	if (!(info->resolution = (int*)malloc(info->dimensions * sizeof(int))) ||
	    !(info->sliceDist = (float*)malloc(info->dimensions * sizeof(float))) ||
	    !(info->origin = (float*)malloc(info->dimensions * sizeof(float)))) {
		datRaw_logError("DatRaw: Failed to allocate memory for description record!\n");
		return 0;
	}
	...
	while (input) {
		...
		case DR_GRID_CARTESIAN:     
			if (strstr(inputLine, "RESOLUTION")) {
				if (info->resolution[0] != -1) {
    ...
    case DR_GRID_CARTESIAN:
        for (i = 0; i < info->dimensions; i++) {
            ...
            if (info->origin[i] == FLT_MAX) {
                datRaw_logWarning("DatRaw Warning: %d. origin in %s invalid\n-> set to 0! \n", i, datfile);
                info->origin[i] = 0.f;
            }
        }
        break;
...
void datRaw_printInfo(const DatRawFileInfo *info)
{
    ...
    case DR_GRID_CARTESIAN:
        ...
        for (i = 0; i < info->dimensions; i++) {
            fprintf(stdout, "%g ", info->origin[i]);
        }
        fprintf(stdout, "\n");          
        break; 
    case DR_GRID_TETRAHEDRAL:
...
unsigned long datRaw_getRecordSize(const DatRawFileInfo *info, int format)
{
    unsigned long dataRecSize = 1;
...
unsigned long datRaw_getElementCount(const DatRawFileInfo *info)
{
	int i;
	unsigned long size;
...
unsigned long datRaw_getBufferSize(const DatRawFileInfo *info, int format)
{
    unsigned long size;
...
{
	unsigned long storageSize, memSize;
	DR_UCHAR *buf;
	int allocateMem;
		...
		DatRawOptionalField **optionalFields)
{
	...
	case DR_GRID_CARTESIAN:
		...
		fprintf(hf, "\n");
		fprintf(hf, "ORIGIN: %f", info->origin[0]);
		for (i = 1; i < info->dimensions; i++) {
			fprintf(hf, " %f", info->origin[i]);
		}
		fprintf(hf, "\n");
		break;
    case DR_GRID_TETRAHEDRAL:
						...
						...)
{
	...
	case DR_GRID_CARTESIAN:
		...
		origin = va_arg(argp, const float*);
		sliceDist = va_arg(argp, const float*);
		resolution = va_arg(argp, const int*);
		va_end(argp);
		if (!(info->origin = (float*)malloc(info->dimensions * sizeof(float))) || 
			!(info->sliceDist = (float*)malloc(info->dimensions * sizeof(float))) || 
	    	!(info->resolution = (int*)malloc(info->dimensions * sizeof(int)))) {
			datRaw_logError("DatRaw: Failed to allocate memory for description record!\n");
			return 0;
		}
		memcpy(info->origin, origin, info->dimensions * sizeof(float));
                     ...
                     ...)
{
	...
	const float *sliceDist;
	const float *origin;
	int i, numTetrahedra, numVertices;
	va_list argp;
	...
	case DR_GRID_CARTESIAN:
		...
		va_end(argp);
		for (i = 0; i < info->dimensions; i++) {
		  if (origin && info->origin[i] != origin[i]) {
				return 0;
			}
...
int datRaw_copyInfo(DatRawFileInfo *newinfo, DatRawFileInfo *oldinfo)
{
	...
	*newinfo = *oldinfo;
	if (!(newinfo->descFileName = dupstr(oldinfo->descFileName)) ||
		...
		!(newinfo->origin = (float*)malloc(oldinfo->dimensions*sizeof(float))) ||
	...
	for (i = 0; i < oldinfo->dimensions; i++) {
	    newinfo->origin[i] = oldinfo->origin[i];
