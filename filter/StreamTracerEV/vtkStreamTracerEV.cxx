...
vtkObjectFactoryNewMacro(vtkStreamTracerEV)
vtkCxxSetObjectMacro(vtkStreamTracerEV,Integrator,vtkInitialValueProblemSolver);
vtkCxxSetObjectMacro(vtkStreamTracerEV,InterpolatorPrototype,vtkAbstractInterpolatedVelocityField);

const double vtkStreamTracerEV::EPSILON = 1.0E-12;
...
vtkStreamTracerEV::vtkStreamTracerEV()
{
  ...
  // by process active cell vectors
  this->SetInputArrayToProcess(1,0,0,vtkDataObject::FIELD_ASSOCIATION_CELLS,
                               vtkDataSetAttributes::VECTORS);

  this->HasMatchingPointAttributes = true;
  
  this->UseCellVectors = true;
}
...
void vtkStreamTracerEV::SetSourceConnection(vtkAlgorithmOutput* algOutput)
{
  this->SetInputConnection(1, algOutput);
}

void vtkStreamTracerEV::SetSourceData(vtkDataSet *source)
{
  this->SetInputData(1, source);
}

vtkDataSet *vtkStreamTracerEV::GetSource()
{
...
int vtkStreamTracerEV::GetIntegratorType()
{
  if (!this->Integrator)
    {
    return NONE;
...
void vtkStreamTracerEV::SetInterpolatorTypeToDataSetPointLocator()
{
  this->SetInterpolatorType
    (  static_cast<int> ( INTERPOLATOR_WITH_DATASET_POINT_LOCATOR )  );
}

void vtkStreamTracerEV::SetInterpolatorTypeToCellLocator()
{
  this->SetInterpolatorType
    (  static_cast<int> ( INTERPOLATOR_WITH_CELL_LOCATOR )  );
}

void vtkStreamTracerEV::SetInterpolatorType( int interpType )
{
...
void vtkStreamTracerEV::SetIntegratorType(int type)
{
...
void vtkStreamTracerEV::SetIntegrationStepUnit( int unit )
{
...
double vtkStreamTracerEV::ConvertToLength(
...
double vtkStreamTracerEV::ConvertToLength(
  vtkStreamTracerEV::IntervalInformation& interval, double cellLength )
{
  return ConvertToLength( interval.Interval, interval.Unit, cellLength );
}

void vtkStreamTracerEV::ConvertIntervals( double& step, double& minStep,
...
void vtkStreamTracerEV::CalculateVorticity(vtkGenericCell* cell,
                                         double pcoords[3],
...
void vtkStreamTracerEV::InitializeSeeds(vtkDataArray*& seeds,
                                      vtkIdList*& seedIds,
...
int vtkStreamTracerEV::SetupOutput(vtkInformation* inInfo,
...
int vtkStreamTracerEV::RequestData(
  ...
  vtkInformationVector *outputVector)
{
  ...
  if (seeds)
    ...
    vtkDataArray *pcaVectors = this->GetInputArrayToProcess(1, input0);
    if (vectors)
      {
      const char *vecName = vectors->GetName();
      double propagation = 0;
      vtkIdType numSteps = 0;
      this->Integrate(input0->GetPointData(), pcaVectors, output,
                      seeds, seedIds,
...
int vtkStreamTracerEV::CheckInputs(vtkAbstractInterpolatedVelocityField*& func,
                                   int* maxCellSize)
{
  if (!this->InputData)
    {
...
void vtkStreamTracerEV::Integrate(vtkPointData *input0Data,
                                vtkDataArray *pcaVectors,
                                vtkPolyData* output,
                                ...
                                vtkIdType& inNumSteps)
{
  ...
  for(int currentLine = 0; currentLine < numLines; currentLine++)
    ...
    // direction of previous integration step
    // this is required to eventually flip eigen vectors
    double dir[3];

    // init with zero which doesn't switch directions at all
    ...
    if (!func->FunctionValues(point1, velocity))
      ...
      {
        // if given, use mean velocity direction per cell for initial alignment of traces
        if( this->UseCellVectors && pcaVectors )
          {
          vtkIdType cellId = func->GetLastCellId();
          pcaVectors->GetTuple(cellId, dir);
          }
      }
    ...
    if (this->ComputeVorticity)
      ...
      if(vecType == vtkDataObject::POINT)
        ...
        vtkStreamTracerEV::CalculateVorticity(cell, pcoords, cellVectors, vort);
        }
      else
        {
        vort[0] = 0;
    ...
    while ( propagation < this->MaximumPropagation )
      ...
      // Tell previous step direction to field interpolator
      func->SetLastDirection(dir);
      ...
      for(i=0; i<3; i++)
        {
        dir[i] = (point2[i] - point1[i]) * direction;
        point1[i] = point2[i];
        }
...
void vtkStreamTracerEV::GenerateNormals(vtkPolyData* output, double* firstNormal,
                                      const char *vecName)
{
  // Useful pointers
  vtkDataSetAttributes* outputPD = output->GetPointData();
...
// than Integrate.
void vtkStreamTracerEV::SimpleIntegrate(double seed[3],
                                      double lastPoint[3],
                                      double stepSize,
                                      vtkAbstractInterpolatedVelocityField* func)
{
...
int vtkStreamTracerEV::FillInputPortInformation(int port, vtkInformation *info)
{
  if (port == 0)
    {
...
void vtkStreamTracerEV::PrintSelf(ostream& os, vtkIndent indent)
{
...
vtkExecutive* vtkStreamTracerEV::CreateDefaultExecutive()
{
