...
#include "vtkInterpolatedVelocityFieldPCA.h"
...
vtkObjectFactoryNewMacro(vtkStreamTracerPCA)
vtkCxxSetObjectMacro(vtkStreamTracerPCA,Integrator,vtkInitialValueProblemSolver);
vtkCxxSetObjectMacro(vtkStreamTracerPCA,InterpolatorPrototype,vtkAbstractInterpolatedVelocityField);

const double vtkStreamTracerPCA::EPSILON = 1.0E-12;
...
vtkStreamTracerPCA::vtkStreamTracerPCA()
{
...
void vtkStreamTracerPCA::SetSourceConnection(vtkAlgorithmOutput* algOutput)
{
  this->SetInputConnection(1, algOutput);
}

void vtkStreamTracerPCA::SetSourceData(vtkDataSet *source)
{
  this->SetInputData(1, source);
}

vtkDataSet *vtkStreamTracerPCA::GetSource()
{
...
int vtkStreamTracerPCA::GetIntegratorType()
{
  if (!this->Integrator)
    {
    return NONE;
...
void vtkStreamTracerPCA::SetInterpolatorTypeToDataSetPointLocator()
{
  this->SetInterpolatorType
    (  static_cast<int> ( INTERPOLATOR_WITH_DATASET_POINT_LOCATOR )  );
}

void vtkStreamTracerPCA::SetInterpolatorTypeToCellLocator()
{
  this->SetInterpolatorType
    (  static_cast<int> ( INTERPOLATOR_WITH_CELL_LOCATOR )  );
}

void vtkStreamTracerPCA::SetInterpolatorType( int interpType )
{
...
void vtkStreamTracerPCA::SetIntegratorType(int type)
{
...
void vtkStreamTracerPCA::SetIntegrationStepUnit( int unit )
{
...
double vtkStreamTracerPCA::ConvertToLength(
...
double vtkStreamTracerPCA::ConvertToLength(
  vtkStreamTracerPCA::IntervalInformation& interval, double cellLength )
{
  return ConvertToLength( interval.Interval, interval.Unit, cellLength );
}

void vtkStreamTracerPCA::ConvertIntervals( double& step, double& minStep,
...
void vtkStreamTracerPCA::CalculateVorticity(vtkGenericCell* cell,
                                         double pcoords[3],
...
void vtkStreamTracerPCA::InitializeSeeds(vtkDataArray*& seeds,
                                      vtkIdList*& seedIds,
...
int vtkStreamTracerPCA::SetupOutput(vtkInformation* inInfo,
...
int vtkStreamTracerPCA::RequestData(
...
// TODO: set new field interpolator here
int vtkStreamTracerPCA::CheckInputs(vtkAbstractInterpolatedVelocityField*& func,
                                   int* maxCellSize)
{
  if (!this->InputData)
    {
...
void vtkStreamTracerPCA::Integrate(vtkPointData *input0Data,
                                vtkPolyData* output,
  ...
  for(int currentLine = 0; currentLine < numLines; currentLine++)
    ...
    if (this->ComputeVorticity)
      ...
      if(vecType == vtkDataObject::POINT)
        ...
        vtkStreamTracerPCA::CalculateVorticity(cell, pcoords, cellVectors, vort);
        }
      else
        {
        vort[0] = 0;
...
void vtkStreamTracerPCA::GenerateNormals(vtkPolyData* output, double* firstNormal,
                                      const char *vecName)
{
  // Useful pointers
  vtkDataSetAttributes* outputPD = output->GetPointData();
...
// than Integrate.
void vtkStreamTracerPCA::SimpleIntegrate(double seed[3],
                                      double lastPoint[3],
                                      double stepSize,
                                      vtkAbstractInterpolatedVelocityField* func)
{
...
int vtkStreamTracerPCA::FillInputPortInformation(int port, vtkInformation *info)
{
  if (port == 0)
    {
...
void vtkStreamTracerPCA::PrintSelf(ostream& os, vtkIndent indent)
{
...
vtkExecutive* vtkStreamTracerPCA::CreateDefaultExecutive()
{
