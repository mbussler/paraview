  ...
  ( vtkDataSet * dataset, double * x, double * f )
{
  ...
  // HACK check if dataset has PCA vector array and use it to align vectors..
  vtkDataArray* vecPCA = dataset->GetCellData()->GetArray("vec_pca");
  if( !vecPCA )
    vecPCA = dataset->GetCellData()->GetArray("ev0");
  ...
  if ( this->LastCellId >= 0 )
    ...
    if(this->VectorsType==vtkDataObject::POINT)
      ...
      if( vecPCA )
        {
        double mainDirection[3];
        vecPCA->GetTuple(LastCellId, mainDirection);
        ...
        for ( j = 0; j < numPts; j ++ )
          ...
          if( vtkMath::Dot(vec, mainDirection) < 0)
            {
